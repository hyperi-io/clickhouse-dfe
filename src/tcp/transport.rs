...
//!
//! The `Tls` variant is feature-gated behind `tls`; without that feature
//! it does not exist at compile time and pattern matching is exhaustive
//! on `Plain` alone.
...
/// The connection actor pattern-matches internally; external users treat
/// the value as an opaque `AsyncRead + AsyncWrite` handle.
