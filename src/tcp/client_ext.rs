...
//! surface and the connection-actor primitives. They are deliberately
//! small and stateless -- all heavy lifting (cancel-on-drop, full-duplex
...
//!
//! A reader of `TcpClient` does not need to know about `NativePool`,
//! `ConnectionHandle`, or the actor's `execute_*` shape.
//!
...
pub async fn execute_stream_via_pool(
    ...
    run_with_retry(pool, retry, |conn| {
        ...
        async move {
            let cursor = conn.execute_stream_cursor(query_id, query, settings).await;
            match cursor {
                Ok(c) => Ok(c),
                Err(e) => {
                    conn.poison();
...
/// that, callers must call [`Self::finish`] or [`Self::abort`] (which
/// poisons and drops).
