...
// Private, like upstream's own `src/native/` keeps `BlockReader` and
// `BlockWriter`. The types a caller needs are re-exported below, so the
// module layout stays free to move without a breaking change.
